...
const ShareIconX = () => ( <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="text-white"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg> );
const ShareIconLine = () => ( <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="text-white"><path d="M16.48,8.321a1.043,1.043,0,0,0-1.033,1.054v.172a.87.87,0,0,1-.266.658,1.033,1.033,0,0,1-.759.29h-3.2a.3.3,0,0,0-.3.3v3.2a.3.3,0,0,0,.3.3h3.2a1.043,1.043,0,0,1,1.033,1.054v.172a.87.87,0,0,1-.266.658,1.033,1.033,0,0,1-.759.29H7.833a.3.3,0,0,0-.3.3v3.2a.3.3,0,0,0,.3.3h8.646a4.521,4.521,0,0,0,4.521-4.521V12.842A4.521,4.521,0,0,0,16.48,8.321ZM7.521,8.321a4.521,4.521,0,0,0-4.521,4.521v4.479a.3.3,0,0,0,.3.3H7.521a1.043,1.043,0,0,1,1.033-1.054V16.4a.87.87,0,0,1,.266-.658,1.033,1.033,0,0,1,.759-.29h3.2a.3.3,0,0,0,.3-.3v-3.2a.3.3,0,0,0-.3-.3H9.579a1.043,1.043,0,0,1-1.033-1.054V9.375a.87.87,0,0,1-.266-.658,1.033,1.033,0,0,1-.759-.29Z" /></svg> );
const ShareIconFacebook = () => ( <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="text-white"><path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878V14.89h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33V21.878C18.343 21.128 22 16.991 22 12z" /></svg> );
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white/80 backdrop-blur-sm p-3 rounded-lg border border-gray-200 shadow-lg">
        ...
        <p className="text-sm text-gray-500">メジャー度: {Math.round(payload[0].payload.score)}%</p>
      </div>
    );
  }
  return null;
...
export default function ResultsView({ 
  ...
  const getShareText = (score) => {
    ...
    else if (score <= 20) { comment = `かなりニッチな領域に足を踏み入れてますね。理解者を見つけるのは至難の業かも。`; }
    else if (score <= 30) { comment = `一般的ではない嗜好をお持ちのようで。でもそこが味わい深いところなのでは？`; }
    else if (score <= 40) { comment = `少し人と違った感性の持ち主。それが個性というものかもしれない。`; }
    else if (score <= 50) { comment = `中間地点。王道も通るし、時には脇道も楽しむバランス型ですね。`; }
    else if (score <= 60) { comment = `やや主流寄りの嗜好。多くの人と話が合いそうで何より。`; }
    else if (score <= 70) { comment = `結構メジャーな路線。飲み会での話題に困らなそうです。`; }
  ...
  const shareUrl = displayedUserId 
    ? `${window.location.origin}${window.location.pathname}?resultId=${displayedUserId}`
  ...
  const isOwnResult = currentUserId && currentUserId === displayedUserId;
  const isSharedView = !isOwnResult;
      ...
      <div className="bg-white/80 backdrop-blur-xl border border-pink-200/80 rounded-3xl shadow-2xl p-6 md:p-10 w-full max-w-4xl mx-auto fade-in">
        ...
        <section className="text-center mb-12 fade-in" style={{ animationDelay: '200ms' }}>
            ...
            <div className="flex justify-center items-center flex-wrap gap-4">
                <a href={twitterShareUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-16 h-16 bg-black rounded-full shadow-lg transform transition-transform hover:scale-110" aria-label="Share on X"><ShareIconX /></a>
                <a href={lineShareUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-16 h-16 bg-green-500 rounded-full shadow-lg transform transition-transform hover:scale-110" aria-label="Share on Line"><ShareIconLine /></a>
                <a href={facebookShareUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-16 h-16 bg-blue-600 rounded-full shadow-lg transform transition-transform hover:scale-110" aria-label="Share on Facebook"><ShareIconFacebook /></a>
            </div>
            <div className="mt-6 flex justify-center flex-wrap gap-4">
                ...
                <button onClick={() => copyToClipboard(shareUrl, 'url')} className="relative bg-gray-600 text-white font-semibold py-2 px-5 rounded-lg shadow-md hover:bg-gray-700 transition-colors">
                  {copySuccess === 'url' ? 'コピーしました！' : '結果のリンクをコピー'}
                </button>
            </div>
        </section>
